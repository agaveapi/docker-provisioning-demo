...
for i in $(dirname $0)/../templates/systems/*execution*
do
  #systems-addupdate -F - <<< \
  ...
  # echo "Verifying login credentials work on $AGAVE_USERNAME-$DEMO_MACHINE_NAME-compute..."
  # files-list -S "$AGAVE_USERNAME-$DEMO_MACHINE_NAME-compute" .
done
